<template>
  <ul class="list"
		...
		<li v-for="item in dataList" :key="item.declareId || item.generalId || item.explainId || item.informationId">
			<header @click="toDetail(item.declareId || item.generalId || item.explainId || item.informationId, item.declareStatus)">
				<h2>{{item.declareTitle || item.generalTitle || item.explainTitle || item.informationTitle}}</h2>
			</header>
			<div class="line-container">
				<span>{{item.declareSource || item.generalSource || item.explainSource || item.informationSource}}</span>
				<span>{{formatDate(item.declarePublishTime || item.generalPublishTime || item.explainPublishTime || item.informationPublishTime)}}</span>
			</div>
			<ul class="tag">
				<li v-for="tag in (item.declareTagList || item.tags || item.explainTagList)" :key="tag.commonId">{{tag.commonName}}</li>
			</ul>
		</li>
